...
namespace utf {
  ...
  encoding_type detect_encoding(std::string &input);

  // determine whether a string is valid in a particular encoding
  bool is_valid(std::string &input, encoding_type encoding);
  ...
  uint32_t get_char(std::string &input, size_t pos, encoding_type encoding);
